...
#Preview("Badges & Buttons") {
    VStack(spacing: 16) {
        HStack(spacing: 8) {
            StoreBadgeView(store: "steam")
            StoreBadgeView(store: "epic")
            StoreBadgeView(store: "gog")
            StoreBadgeView(store: "microsoft")
        }
        HStack(spacing: 8) {
            DiscountChipView(pct: 33)
            DiscountChipView(pct: 50)
        }
        SunsetButton(title: "View deal", icon: "arrow.right") {}
        GlassButton(title: "Cancel") {}
        PriceLabelView(priceCents: 3999, currency: "USD", originalCents: 5999)
        PriceLabelView(priceCents: 149900, currency: "COP", originalCents: 299900)
    }
    .padding()
    .background(GSTheme.dark.bg)
    .environment(\.gsTheme, GSTheme.dark)
}

#Preview("Game Cards") {
    let mock = GS_MOCK_GAMES[0]
    let cheapest = mock.cheapestStore!
    let game = StorePrice(
...
#Preview("Tab Bar") {
    ZStack(alignment: .bottom) {
        GSTheme.dark.bg.ignoresSafeArea()
