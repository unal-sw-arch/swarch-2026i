...
struct GameDetailView: View {
    ...
    init(
        game: StorePrice? = nil,
        gameName: String? = nil,
        rankedGame: RankedGame? = nil,
        onClose: (() -> Void)? = nil
    ) {
        self.game = game
        self.gameName = gameName
        self.rankedGame = rankedGame
            ...
            HStack(spacing: 10) {
                ...
                Button {
                    ...
                    Image(systemName: wishlisted ? "heart.fill" : "heart")
                        .font(.system(size: 15, weight: .semibold))
                        .foregroundStyle(wishlisted ? theme.error : theme.onSurface)
                        .frame(width: 40, height: 40)
                        .background(theme.surfaceHigh, in: Circle())
                }
                .buttonStyle(.plain)
    ...
    private func toggleWishlist() {
        ...
        if wishlisted {
            ...
            Task {
                if let saved = try? await GatewayService.shared.addToWishlist(name: displayName, slug: slug) {
...
#Preview("Game Detail – Light") {
    let appState = AppState()
    appState.currentUser = AuthUser(id: "1", name: "Miguel", email: "miguel@example.com")
    appState.isDarkMode = false
    let mock = GS_MOCK_GAMES[1]
    return NavigationStack {
        GameDetailView(gameName: mock.name)
    }
    .environment(appState)
    .environment(\.gsTheme, GSTheme.light)
    .modelContainer(for: [WishlistEntry.self, PriceAlert.self, RecentSearch.self, CachedGame.self], inMemory: true)
}
