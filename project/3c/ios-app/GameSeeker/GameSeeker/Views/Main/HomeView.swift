...
struct HomeView: View {
    ...
    private var topBlurOpacity: Double {
        Double(min(max(-scrollOffset / 56, 0), 1))
    }
    ...
    private var topScrollBlur: some View {
        VStack(spacing: 0) {
            Rectangle()
                ...
                .overlay(
                    LinearGradient(
                        colors: [theme.bg.opacity(0.92), theme.bg.opacity(0.54), .clear],
                        startPoint: .top,
                        endPoint: .bottom
                    )
                )
