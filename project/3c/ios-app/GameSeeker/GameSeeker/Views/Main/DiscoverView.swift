...
struct DiscoverView: View {
    ...
    @State private var outgoingGame: MockGame?
    ...
    var body: some View {
                    ...
                    ZStack {
                        ...
                        if let third = thirdGame {
                            swipeCard(third, zIndex: 1)
                                .frame(width: cardProxy.size.width, height: cardHeight)
                                .offset(y: 18)
                                .opacity(0.28)
                        ...
                        if let outgoingGame {
                            swipeLayer(game: outgoingGame, offset: outgoingOffset)
                                .id("outgoing-\(outgoingGame.slug)")
                                .frame(width: cardProxy.size.width, height: cardHeight)
        ...
        .sheet(isPresented: $showDetail) {
            if let game = detailGame {
                GameDetailView(game: game, onClose: {
                    showDetail = false
                })
            }
        }
    ...
    private func topCard(_ game: MockGame) -> some View {
        swipeLayer(game: game, offset: dragOffset)
            .gesture(
            DragGesture()
                .onChanged { value in
    ...
    private func swipeLayer(game: MockGame, offset: CGSize) -> some View {
        let rotation = Double(offset.width / 20)
        let wishStampOpacity = max(0, Double(offset.width) / 80)
        let passStampOpacity = max(0, Double(-offset.width) / 80)
    ...
    private func commitSwipe(direction: CGFloat) {
        guard !isAnimatingOut, let swipedGame = currentGame else { return }
        ...
        if direction > 0 {
            // Add to wishlist
            let slug = swipedGame.slug
            let entry = WishlistEntry(
                gameSlug: slug,
                gameName: swipedGame.name,
                imageUrl: swipedGame.imageUrl,
                priceCents: swipedGame.cheapestStore?.priceCents ?? 0,
                currency: swipedGame.cheapestStore?.currency ?? "USD"
            )
            modelContext.insert(entry)
            try? modelContext.save()
            Task {
                if let saved = try? await GatewayService.shared.addToWishlist(name: swipedGame.name, slug: slug) {
        ...
        let nextIndex = currentIndex < games.count - 1 ? currentIndex + 1 : 0
        let startingOffset = dragOffset

        isAnimatingOut = true
        outgoingGame = swipedGame
        outgoingOffset = startingOffset
